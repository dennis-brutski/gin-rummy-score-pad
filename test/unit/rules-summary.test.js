...
test('rulesOf merges snapshot over defaults', () => {
  ...
  assert.equal(rulesOf({}).undercutBonus, 10);
});

test('applyRulesToGame rescores every hand from raw points', () => {
  let g = applyHandToGame(newGame('a', 'b'), { type: 'gin', winner: 0, points: 20 }); // 40
...
test('lowering target below a current score finishes the game (state level; UI locks this)', () => {
  let g = applyHandToGame(newGame('a', 'b'), { type: 'gin', winner: 0, points: 60 }); // 80
...
test('summary applies game and box bonuses', () => {
  ...
  g = applyHandToGame(g, { type: 'gin', winner: 0, points: 98 }); // a: 118, finished
  const s = summaryForGame(g);
  assert.equal(s.winner, 0);
  assert.equal(s.shutout, false);
  assert.equal(s.gameBonus, 100);
  assert.equal(s.boxBonus, 20);        // 1 box × 20
  assert.equal(s.loserBoxBonus, 20);
  assert.deepEqual(s.finals, [118 + 100 + 20, 20 + 20]); // [238, 40]
});
...
test('summary adds shutout bonus when loser has zero boxes', () => {
  ...
  assert.deepEqual(s.finals, [118 + 100 + 20 + 100, 0]); // [338, 0]
});
